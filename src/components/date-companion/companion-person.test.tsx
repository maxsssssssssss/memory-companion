...
describe("CompanionPerson", () => {
  it("keeps the four deterministic fact cards and uses only one supplied relationship observation", () => {
    render(
      <CompanionPerson
        currentInteraction={null}
        person={person}
        proactiveObservation={<p>服务端返回的关系观察</p>}
        relationship={relationship}
      />
    );

    expect(screen.getByLabelText("关于 Ta 的四类内容")).toBeInTheDocument();
    expect(screen.getAllByText("服务端返回的关系观察")).toHaveLength(1);
    expect(screen.queryByText("还没有足够片段形成观察")).not.toBeInTheDocument();
  });
