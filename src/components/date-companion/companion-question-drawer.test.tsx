...
describe("CompanionQuestionDrawer", () => {
  ...
  it("disables relationship QA until the relationship is ready", () => {
    const onAsk = vi.fn();
    render(
  ...
  it("opens a left-side conversation panel and lets a suggested question use the real QA callback", () => {
    ...
    const composer = screen.getByRole("textbox", { name: "向 Ta 的相处记录提问" });
    expect(composer).toHaveValue("Ta 以前明确提到过哪些在意的事？");
    expect(onAsk).not.toHaveBeenCalled();

    fireEvent.click(screen.getByRole("button", { name: "发送" }));
    expect(onAsk).toHaveBeenCalledWith("Ta 以前明确提到过哪些在意的事？");
  });

  it("shows at most two derived questions plus one stable rule fallback and only fills the composer", () => {
    const onAsk = vi.fn(async () => undefined);
    render(
      <CompanionQuestionDrawer
        answers={[]}
    ...
    expect(screen.getByRole("button", { name: "Ta 当时为什么会特别在意这件事？" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "以前还有哪些相似的相处片段？" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Ta 以前明确提到过哪些在意的事？" })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "第三条不应显示" })).not.toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "Ta 当时为什么会特别在意这件事？" }));
  ...
  it("exposes an explicit current-interaction mode with its own stable labels", () => {
    ...
    expect(screen.getByRole("dialog", { name: "问问这次相处" })).toBeInTheDocument();
    const composer = screen.getByRole("textbox", { name: "针对这次相处提问" });
    fireEvent.change(composer, { target: { value: "这次记录了什么？" } });
    fireEvent.click(screen.getByRole("button", { name: "发送" }));
    expect(onAsk).toHaveBeenCalledWith("这次记录了什么？");
    expect(screen.getByText("只使用这一次相处在本机保存的完整记录。")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "关闭" }));
    expect(screen.queryByRole("dialog", { name: "问问这次相处" })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "问问这次相处" }));
  ...
  it("cancels a streaming question and closes the drawer on Escape", () => {
    ...
    const trigger = screen.getByRole("button", { name: "问问 Ta" });
  ...
  it("renders a completed answer in history with a link to its valid transcript citation", () => {
    const onOpenTranscriptSource = vi.fn(() => false);
    render(
      <CompanionQuestionDrawer
        ...
        validSegmentIds={new Set(["segment-1"])}
        linkableSegmentIds={new Set(["segment-1"])}
        onOpenTranscriptSource={onOpenTranscriptSource}
        onAsk={vi.fn()}
        onCancel={vi.fn()}
      />
    );
    ...
    expect(evidenceItem).not.toHaveAttribute("open");
    expect(screen.getByText("来自和 Ta 的相处记录 · 电影计划")).toBeInTheDocument();
    ...
    fireEvent.click(screen.getByRole("link", { name: "在完整文字稿中查看", hidden: true }));
    expect(onOpenTranscriptSource).toHaveBeenCalledWith("segment-1");
  });
  ...
  it("renders the active question as a conversation turn while streaming and restores it after failure", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "问问 Ta" }));
    expect(screen.getByRole("log", { name: "问问 Ta 对话" })).toHaveTextContent("Ta 这次最在意什么？");
  ...
  it("does not present a citation as valid when any referenced segment is outside this interaction", () => {
    ...
    expect(screen.queryByText("来自和 Ta 的相处记录 · 电影计划")).not.toBeInTheDocument();
    expect(screen.getByText("回答证据").closest("details")).not.toHaveAttribute("open");
    expect(screen.getByText("片段")).toBeInTheDocument();
    expect(screen.getByText("来自和 Ta 的相处记录 · 文字片段")).toBeInTheDocument();
    expect(screen.queryByText(/^E\d+$/u)).not.toBeInTheDocument();
  });

  it("shows a snapshot-only quote without creating an invalid transcript link", () => {
    render(
      <CompanionQuestionDrawer
        answers={[completedAnswer]}
        enabled
    ...
    expect(screen.queryByRole("link", { name: "在完整文字稿中查看", hidden: true })).not.toBeInTheDocument();
  });
});
