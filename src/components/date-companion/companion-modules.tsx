  ...
  return (
    <main className={styles.modulePage}>
      ...
      <section className={styles.moduleIntro}>
        ...
        <h1>今天，你想从哪里开始？</h1>
        <span>{dailyReflectionEnabled
          ? "先选择一个空间。日常复盘目前仅作内部开放。"
          : "先选择一个空间。第一版只开放约会陪伴。"}</span>
      </section>
        ...
        {dailyReflectionEnabled ? (
          <Link className={`${styles.moduleCard} ${styles.chatModule} ${styles.enabledModule}`} href="/date-companion/reflection">
            ...
            <div className={styles.moduleCopy}>
              <p>FOR EVERYDAY</p>
              <h2>日常复盘</h2>
              <span>上传一段已有录音，先查看完整文字和待确认内容。</span>
            </div>
            <div className={styles.moduleMeta}>
              <span>上传已有录音 · 查看文字稿 · 等你确认</span>
              <b><span>进入日常复盘</span><span aria-hidden="true">↗</span></b>
            </div>
          </Link>
        ) : (
