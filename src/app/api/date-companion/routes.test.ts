...
const state = vi.hoisted(() => ({
  ...
  deleteVoiceprintCandidates: vi.fn(async () => 0),
  deleteOwnerReviewCandidates: vi.fn(async () => 0),
  resolvePersonSourceCatalog: vi.fn(),
  deleteRetainedMemory: vi.fn(async (): Promise<{
    memoryDeleted: boolean;
    indexRefresh: "not_required" | "enqueued" | "failed_best_effort";
  }> => ({
    memoryDeleted: true,
    indexRefresh: "not_required" as const
  }))
}));
...
vi.mock("@/lib/server/date-companion/person-source-catalog", async (importOriginal) => ({
  ...
  resolveProductionDateCompanionPersonSourceCatalog: state.resolvePersonSourceCatalog
}));
...
vi.mock("@/lib/server/speaker-identity/voiceprint-training-candidates", async (importOriginal) => ({
  ...(await importOriginal<typeof import(
    "@/lib/server/speaker-identity/voiceprint-training-candidates"
  )>()),
  deleteVoiceprintTrainingCandidatesForUpload: state.deleteVoiceprintCandidates
}));
...
vi.mock("@/lib/server/memory/owner-review", async (importOriginal) => ({
  ...
  deleteMemoryOwnerReviewCandidatesForUpload: state.deleteOwnerReviewCandidates
}));
...
vi.mock("@/lib/server/memory/upload-deletion", async (importOriginal) => ({
  ...
  deleteMemoryUploadAndRefreshIndex: state.deleteRetainedMemory
}));
...
describe("Date Companion API isolation", () => {
  ...
  beforeEach(() => {
    ...
    state.deleteVoiceprintCandidates.mockReset();
    state.deleteVoiceprintCandidates.mockResolvedValue(0);
    state.deleteOwnerReviewCandidates.mockReset();
    state.deleteOwnerReviewCandidates.mockResolvedValue(0);
    state.resolvePersonSourceCatalog.mockReset();
    state.resolvePersonSourceCatalog.mockImplementation((input: {
      accountId: string;
      relationshipId: string;
    }) => {
      repository.getRelationshipView(input.accountId, input.relationshipId);
      return {
    ...
    state.deleteRetainedMemory.mockReset();
    state.deleteRetainedMemory.mockResolvedValue({
      memoryDeleted: true,
  ...
  it("keeps recap, promise, search and delete routes authenticated, user-scoped and versioned", async () => {
    ...
    await expect(ownerSearch.json()).resolves.toEqual({ results: [] });
    expect(state.resolvePersonSourceCatalog).toHaveBeenCalledWith({
      accountId: "user_a",
      relationshipId: relationship.id
    });
    ...
    expect(state.deleteVoiceprintCandidates).not.toHaveBeenCalled();
    expect(state.deleteOwnerReviewCandidates).not.toHaveBeenCalled();
    ...
    expect(state.deleteVoiceprintCandidates).toHaveBeenCalledWith({
      store: state.store,
      uploadId: "upload_routes",
      uploadsRootDir: ".data/uploads"
    });
    expect(state.deleteOwnerReviewCandidates).toHaveBeenCalledWith({
      store: state.store,
      uploadId: "upload_routes",
      uploadsRootDir: ".data/uploads"
    });
    expect(state.deleteRetainedMemory).not.toHaveBeenCalled();
  ...
  it("retains the interaction when explicit-delete voice cleanup fails", async () => {
    ...
    repository.markUploadSourceState(
      "user_cleanup",
      "upload_cleanup_failure",
      "server_cleaned"
    );
    state.deleteVoiceprintCandidates.mockRejectedValueOnce(new Error("cleanup failed"));
    ...
    try {
      ...
      expect(state.deleteRetainedMemory).toHaveBeenCalledOnce();
      expect(state.deleteOwnerReviewCandidates).toHaveBeenCalledOnce();
      expect((state.store as { delete: ReturnType<typeof vi.fn> }).delete).not.toHaveBeenCalled();
      expect((state.store as { list: ReturnType<typeof vi.fn> }).list).not.toHaveBeenCalled();
    } finally {
  ...
  it("retains the interaction when explicit-delete Owner Review cleanup fails", async () => {
    const relationship = repository.createOrGetRelationship("user_owner_cleanup", "Ta").relationship;
    const imported = repository.importInteraction({
      userId: "user_owner_cleanup",
      relationshipId: relationship.id,
      sourceUploadId: "upload_owner_cleanup_failure",
      recordingDate: "2026-08-05",
      originalName: "cleanup.wav",
      participants: [],
      recapCandidates: []
    ...
    repository.markUploadSourceState(
      "user_owner_cleanup",
      "upload_owner_cleanup_failure",
      "server_cleaned"
    );
    state.deleteOwnerReviewCandidates.mockRejectedValueOnce(new Error("cleanup failed"));
    const consoleError = vi.spyOn(console, "error").mockImplementation(() => undefined);
    try {
      const response = (await deleteInteraction(new Request(
        `http://localhost/api/date-companion/interactions/${imported.interactionId}`,
        {
          method: "DELETE",
          headers: { "x-test-user": "user_owner_cleanup", "if-match": "0" }
        }
      ), { params: Promise.resolve({ interactionId: imported.interactionId }) }))!;
      expect(response.status).toBe(500);
      await expect(response.json()).resolves.toEqual({
        error: "interaction_owner_cleanup_failed",
        deleted: false,
        retryable: true
      });
      expect(repository.getRelationshipView("user_owner_cleanup", relationship.id)
        .interactions).toHaveLength(1);
      expect(state.deleteRetainedMemory).toHaveBeenCalledOnce();
      expect(state.deleteVoiceprintCandidates).not.toHaveBeenCalled();
      expect((state.store as { delete: ReturnType<typeof vi.fn> }).delete).not.toHaveBeenCalled();
  ...
  it("deletes server-cleaned Memory before the interaction and keeps repeated DELETE account-scoped", async () => {
    const relationship = repository.createOrGetRelationship("user_memory", "Ta").relationship;
    const imported = repository.importInteraction({
      userId: "user_memory",
      relationshipId: relationship.id,
      sourceUploadId: "upload_memory",
      ...
      recapCandidates: [{
        ...
        evidence: [{
          ...
          sourceSegmentId: "segment_memory",
          startSeconds: 0,
          endSeconds: 2,
          speakerId: "speaker_0",
          quote: "保留的内容"
    ...
    repository.markUploadSourceState("user_memory", "upload_memory", "server_cleaned");
    const order: string[] = [];
    state.deleteOwnerReviewCandidates.mockImplementationOnce(async () => {
      order.push("owner");
      return 0;
    });
    state.deleteRetainedMemory.mockImplementationOnce(async () => {
      order.push("memory");
      return { memoryDeleted: true, indexRefresh: "enqueued" };
    });
    state.deleteVoiceprintCandidates.mockImplementationOnce(async () => {
      order.push("voiceprint");
      return 0;
    });
    const originalDelete = repository.deleteInteraction.bind(repository);
    vi.spyOn(repository, "deleteInteraction").mockImplementationOnce((...args) => {
      order.push("date_companion");
      return originalDelete(...args);
    });
    const call = (userId: string) => deleteInteraction(new Request(
      `http://localhost/api/date-companion/interactions/${imported.interactionId}`,
      { method: "DELETE", headers: { "x-test-user": userId, "if-match": "0" } }
    ), { params: Promise.resolve({ interactionId: imported.interactionId }) });

    const response = (await call("user_memory"))!;
    expect(response.status).toBe(200);
    expect(order).toEqual(["memory", "owner", "voiceprint", "date_companion"]);
    expect(state.deleteRetainedMemory).toHaveBeenCalledWith({
      userId: "user_memory",
      uploadId: "upload_memory",
      indexRefreshFailure: "throw"
    });
    expect(database.prepare(`
      SELECT COUNT(*) AS count FROM dc_evidence_snapshots WHERE user_id = 'user_memory'
    `).get()).toEqual({ count: 0 });
    expect((await call("user_memory"))!.status).toBe(404);
    expect((await call("user_other"))!.status).toBe(404);
  });

  it.each([
    ["Memory", new MemoryUploadDeletionError("memory_upload_delete_failed"),
      "interaction_memory_cleanup_failed"],
    ["index", new MemoryUploadDeletionError("memory_index_refresh_failed"),
      "interaction_memory_index_refresh_failed"]
  ])("retains server-cleaned DC Evidence when %s cleanup fails and completes on retry",
    async (_kind, failure, expectedCode) => {
      const relationship = repository.createOrGetRelationship(`user_${_kind}`, "Ta").relationship;
      const imported = repository.importInteraction({
        userId: `user_${_kind}`,
        relationshipId: relationship.id,
        sourceUploadId: `upload_${_kind}`,
        recordingDate: "2026-08-11",
        originalName: "retry.wav",
        participants: [],
        recapCandidates: [{
          kind: "mentioned",
          proposedText: "等待重试",
          sortOrder: 0,
          evidence: [{
            uploadId: `upload_${_kind}`,
            sourceSegmentId: `segment_${_kind}`,
            startSeconds: 0,
            endSeconds: 1,
            quote: "等待重试"
          }]
      ...
      repository.markUploadSourceState(`user_${_kind}`, `upload_${_kind}`, "server_cleaned");
      state.deleteRetainedMemory.mockRejectedValueOnce(failure);
      const consoleError = vi.spyOn(console, "error").mockImplementation(() => undefined);
      const call = () => deleteInteraction(new Request(
        `http://localhost/api/date-companion/interactions/${imported.interactionId}`,
        { method: "DELETE", headers: { "x-test-user": `user_${_kind}`, "if-match": "0" } }
      ), { params: Promise.resolve({ interactionId: imported.interactionId }) });
      try {
        ...
        expect(repository.getRelationshipView(`user_${_kind}`, relationship.id).interactions)
          .toHaveLength(1);
        expect(state.deleteOwnerReviewCandidates).not.toHaveBeenCalled();
        expect(state.deleteVoiceprintCandidates).not.toHaveBeenCalled();
        expect((state.store as { delete: ReturnType<typeof vi.fn> }).delete)
          .not.toHaveBeenCalled();
        expect((state.store as { list: ReturnType<typeof vi.fn> }).list)
          .not.toHaveBeenCalled();
        expect(database.prepare(`
          SELECT COUNT(*) AS count FROM dc_evidence_snapshots WHERE user_id = ?
        `).get(`user_${_kind}`)).toEqual({ count: 1 });

        expect((await call())!.status).toBe(200);
        expect(state.deleteRetainedMemory).toHaveBeenCalledTimes(2);
        expect(repository.getRelationshipView(`user_${_kind}`, relationship.id).interactions)
          .toEqual([]);
      } finally {
        consoleError.mockRestore();
  ...
  it("replays idempotently after Memory commits but Date Companion deletion crashes", async () => {
    const relationship = repository.createOrGetRelationship("user_crash", "Ta").relationship;
    const imported = repository.importInteraction({
      userId: "user_crash",
      relationshipId: relationship.id,
      sourceUploadId: "upload_crash",
      recordingDate: "2026-08-11",
      originalName: "crash.wav",
      participants: [],
      recapCandidates: []
    });
    repository.markUploadSourceState("user_crash", "upload_crash", "server_cleaned");
    const originalDelete = repository.deleteInteraction.bind(repository);
    vi.spyOn(repository, "deleteInteraction")
      .mockImplementationOnce(() => {
        throw new Error("simulated_dc_delete_crash");
      })
      .mockImplementationOnce((...args) => originalDelete(...args));
    const call = () => deleteInteraction(new Request(
      `http://localhost/api/date-companion/interactions/${imported.interactionId}`,
      { method: "DELETE", headers: { "x-test-user": "user_crash", "if-match": "0" } }
    ), { params: Promise.resolve({ interactionId: imported.interactionId }) });

    const failed = (await call())!;
    expect(failed.status).toBe(500);
    await expect(failed.json()).resolves.toEqual({
      error: "interaction_delete_failed",
      deleted: false,
      retryable: true
    });
    expect(repository.getRelationshipView("user_crash", relationship.id).interactions)
      .toHaveLength(1);
    expect((await call())!.status).toBe(200);
    expect(state.deleteRetainedMemory).toHaveBeenCalledTimes(2);
    expect(repository.getRelationshipView("user_crash", relationship.id).interactions)
