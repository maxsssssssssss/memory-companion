...
export type HybridIndexRetentionPolicy = "off" | "browser_cache";
...
export function resolveHybridIndexRetentionPolicy(
  value = process.env.HYBRID_INDEX_RETENTION_POLICY
): HybridIndexRetentionPolicy {
  const normalized = value?.trim().toLowerCase() || "off";
  if (normalized === "off" || normalized === "browser_cache") {
    return normalized;
  }
  throw new Error(
...
function timeoutForPurpose(
  ...
  return purpose === "index"
    ...
    : environment.HYBRID_EMBEDDING_QUERY_TIMEOUT_MS ?? "5000";
}
...
export function hybridEmbeddingIndexPath(userId: string, dataRoot?: string) {
  return join(
...
export function assertLocalQwen4BConfig(provider: Pick<QwenEmbeddingProvider, "config">) {
  ...
  if (
    ...
    throw new Error(
      "Hybrid QA requires the pinned Qwen3-Embedding-4B revision at 2560 dimensions"
    );
  }
  return config;
}
