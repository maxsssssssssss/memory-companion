...
import {
  ...
  finalizeDateCompanionMemoryAdmission,
  prepareMemoryBridgeInteractionDeletion,
  type DcMemoryAdmissionInput
} from "./memory-bridge-repository";
import { dateCompanionEvidenceDigest } from "./memory-bridge-digest";
...
function recapConfirmationFingerprint(input: {
  ...
  const memoryAdmission = input.memoryAdmission ? {
    mappingVersion: input.memoryAdmission.mappingVersion,
    subjectSuggestionConfirmation: input.memoryAdmission.subjectSuggestionConfirmation ?? null,
...
export class DateCompanionRepository {
  ...
  private evidenceForRecap(userId: string, recapItemId: string) {
    const rows = this.database.prepare(`
      SELECT id, recap_item_id, upload_id, source_segment_id, start_seconds,
             end_seconds, speaker_id, quote, content_digest, created_at
      ...
      for (const candidate of input.recapCandidates) {
        ...
        for (const evidence of candidate.evidence) {
          ...
          const existingSource = this.database.prepare(`
            ...
            FROM dc_evidence_snapshots
            WHERE user_id = ? AND upload_id = ? AND source_segment_id = ?
            LIMIT 1
          `).get(
            input.userId,
            evidence.uploadId,
            evidence.sourceSegmentId
          ) as { content_digest: string | null } | undefined;
          if (existingSource && existingSource.content_digest !== contentDigest) {
            throw new DcConflictError("evidence_source_digest_conflict");
          }
          insertEvidence.run(
            randomUUID(),
            input.userId,
    ...
    const run = this.database.transaction(() => {
      ...
      const confirmationFingerprint = input.finalize
        ? recapConfirmationFingerprint({
            ...
            memoryAdmission: input.memoryAdmission
          })
        : null;
      if (interaction.status === "confirmed") {
        if (
  ...
  searchPersonProjection(
    userId: string,
    relationshipId: string,
    query: string,
    eligibleEvidenceSnapshotIds: readonly string[],
    expectedMapping: {
      version: number;
    ...
    const eligibleIds = [...new Set(
      eligibleEvidenceSnapshotIds.map((id) => id.trim()).filter(Boolean)
    )];
    if (eligibleIds.length === 0) return [];

    const pattern = `%${escapeLike(query.trim())}%`;
    const rows = this.database.prepare(`
      WITH eligible_evidence(id) AS (
      ...
      JOIN dc_interactions i
        ON i.id = r.interaction_id AND i.user_id = r.user_id
      JOIN dc_relationships relationship
        ON relationship.id = i.relationship_id AND relationship.user_id = i.user_id
      JOIN dc_relationship_person_mappings mapping
        ON mapping.relationship_id = i.relationship_id AND mapping.user_id = i.user_id
      WHERE r.user_id = ?
        AND i.relationship_id = ?
        ...
        AND mapping.version = ?
        AND mapping.companion_person_id = ?
        AND i.status = 'confirmed'
        AND r.disposition = 'kept'
        AND EXISTS (
          SELECT 1
          FROM dc_evidence_snapshots e
          INNER JOIN eligible_evidence eligible ON eligible.id = e.id
          WHERE e.recap_item_id = r.id AND e.user_id = r.user_id
        )
        AND NOT EXISTS (
          SELECT 1
          FROM dc_evidence_snapshots e
          LEFT JOIN eligible_evidence eligible ON eligible.id = e.id
          WHERE e.recap_item_id = r.id
            AND e.user_id = r.user_id
    ...
    `).all(
      ...
      expectedMapping.companionPersonId,
      pattern,
      pattern,
    ...
    return rows.map((row) => DcSearchResultSchema.parse({
      ...
      text: row.user_text ?? row.proposed_text,
      recordingDate: row.recording_date,
      evidence: this.evidenceForRecap(userId, row.id).filter((evidence) =>
        eligibleSet.has(evidence.id)
      )
    }));
  }
  ...
  getMemoryRetentionSetting(userId: string) {
    return createDateCompanionMemoryBridgeRepository(this.database)
      .getRetentionSetting(userId);
  }
  ...
  prepareInteractionDeletion(
    ...
    const run = this.database.transaction(() => {
      ...
      prepareMemoryBridgeInteractionDeletion(this.database, userId, interactionId);
      return {
