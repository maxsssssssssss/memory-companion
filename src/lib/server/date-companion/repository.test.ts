...
describe("DateCompanionRepository", () => {
  ...
  it("searches the Person projection only when every returned Evidence is eligible", () => {
    const { relationship, imported } = importDraft();
    const mappedAt = "2026-08-20T00:00:00.000Z";
    database.prepare(`
      INSERT INTO dc_relationship_person_mappings (
        ...
        'mapping_search', 'user_a', ?, 'person_self', 'person_companion',
        'partner', 'confirmed', 1, ?, ?, ?
      )
    `).run(relationship.id, mappedAt, mappedAt, mappedAt);
    let interaction = repository.getRelationshipView(
      "user_a",
      relationship.id
    ).interactions[0];
    repository.updateRecap({
    ...
    const companionItem = interaction.recapItems.find((item) =>
      item.evidence.some((evidence) => evidence.sourceSegmentId === "segment_companion")
    )!;
    const companionEvidenceId = companionItem.evidence[0].id;

    expect(repository.searchPersonProjection(
      "user_a",
      relationship.id,
      "徒步",
      [companionEvidenceId],
      { version: 1, companionPersonId: "person_companion" }
    )).toEqual([expect.objectContaining({
      recapItemId: companionItem.id,
      evidence: [expect.objectContaining({ id: companionEvidenceId })]
    })]);
    expect(repository.searchPersonProjection(
      "user_a",
      relationship.id,
    ...
    const mixed = repository.importInteraction({
      userId: "user_a",
      relationshipId: relationship.id,
      sourceUploadId: "upload_mixed",
      recordingDate: "2026-08-05",
      originalName: "mixed.wav",
      participants: [{ speakerId: "speaker_0" }, { speakerId: "speaker_1" }],
      recapCandidates: [{
        kind: "moment",
        proposedText: "一起核对报价方案",
        sortOrder: 0,
        evidence: [
          {
            uploadId: "upload_mixed",
            sourceSegmentId: "segment_mixed_companion",
            startSeconds: 0,
            endSeconds: 2,
            speakerId: "speaker_1",
            quote: "Ta 提议周五再核对"
          ...
          {
            uploadId: "upload_mixed",
            sourceSegmentId: "segment_mixed_self",
            startSeconds: 2,
            endSeconds: 4,
            speakerId: "speaker_0",
            quote: "报价方案我来整理"
    ...
    const mixedDraft = repository.getRelationshipView(
      "user_a",
      relationship.id
    ).interactions.find((candidate) => candidate.id === mixed.interactionId)!;
    repository.updateRecap({
      userId: "user_a",
      interactionId: mixed.interactionId,
      version: mixedDraft.version,
      assignments: [
        { speakerId: "speaker_0", role: "self" },
        { speakerId: "speaker_1", role: "companion" }
      ],
      mutations: mixedDraft.recapItems.map((item) => ({
        id: item.id,
        version: item.version,
        disposition: "kept" as const
      })),
    ...
    const mixedItem = repository.getRelationshipView(
      "user_a",
      relationship.id
    ).interactions.find((candidate) => candidate.id === mixed.interactionId)!.recapItems[0];
    const mixedCompanionEvidence = mixedItem.evidence.find((evidence) =>
      evidence.sourceSegmentId === "segment_mixed_companion"
    )!;
    expect(repository.searchPersonProjection(
      "user_a",
      relationship.id,
      "报价方案",
      [mixedCompanionEvidence.id],
      { version: 1, companionPersonId: "person_companion" }
    )).toEqual([]);
    expect(repository.searchPersonProjection(
      "user_a",
    ...
    `).run("2026-08-20T00:01:00.000Z", relationship.id);
    expect(repository.searchPersonProjection(
      "user_a",
      relationship.id,
      "徒步",
    ...
    database.prepare(`
      UPDATE dc_relationship_person_mappings
      SET status = 'archived', confirmed_at = NULL, version = 3, updated_at = ?
      WHERE user_id = 'user_a' AND relationship_id = ?
    `).run("2026-08-20T00:02:00.000Z", relationship.id);
    expect(repository.searchPersonProjection(
      "user_a",
      relationship.id,
      "徒步",
      [companionEvidenceId],
      { version: 3, companionPersonId: "person_companion" }
    )).toEqual([]);
    expect(() => repository.searchPersonProjection(
      "user_b",
      relationship.id,
