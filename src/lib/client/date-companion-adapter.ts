...
function evidenceSource(
  ...
  return {
    ...
    kind: "transcript",
    presentation: "direct_quote",
    canOpenTranscript: options.hasLocalDay?.(evidence.uploadId) === true
  };
}
...
function persistentRecapItem(
  ...
  return {
    ...
    sources: item.evidence.map((evidence) => evidenceSource(evidence, recordingDate, options)),
...
function isLongTermEligible(interaction: DcInteractionDetail, item: DcRecapItem): boolean {
  if (interaction.status !== "confirmed" || item.disposition !== "kept" || item.evidence.length === 0) return false;
  ...
  if (item.kind === "mentioned") return roles.every((role) => role === "companion");
  if (item.kind === "promise") return roles.every((role) => role === "self");
  return true;
}
...
function promiseVm(
  ...
  return {
    ...
    sources: promise.evidence.map((evidence) => evidenceSource(evidence, recordingDate, options))
  };
}
...
export function applyDateCompanionRelationshipView(
  ...
  const confirmed = [...view.interactions]
    .filter((interaction) =>
      ...
      interaction.recapItems.some((item) => isLongTermEligible(interaction, item))
    )
  ...
  const confirmedItems = confirmed.flatMap((interaction) =>
    interaction.recapItems
      .filter((item) => isLongTermEligible(interaction, item))
  ...
  const promises = view.promises.map((promise) =>
    promiseVm(promise, dateByRecapItem.get(promise.originatingRecapItemId) ?? view.relationship.createdAt.slice(0, 10), options)
  );
...
export function buildDateCompanionSearchResults(
  ...
  return results.map((result) => ({
    ...
    sources: result.evidence.map((evidence) => evidenceSource(evidence, result.recordingDate, options))
  }));
}
