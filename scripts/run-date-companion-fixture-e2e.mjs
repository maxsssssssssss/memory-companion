...
const fixtureTsconfigPath = resolve(workspaceDir, relativeDataDir, "next-tsconfig.json");
...
if (!requiresFixtureAudio) {
  progress(0, 5, "UI-only fixture does not create or upload synthetic audio");
} else {
...
const serverEnv = {
  ...
  DAILY_BRIEF_E2E_DIST_DIR: `${relativeDataDir}/next-dist`,
  DAILY_BRIEF_E2E_TSCONFIG: `${relativeDataDir}/next-tsconfig.json`,
  ...
  DAILY_BRIEF_TOY_SYNC_ENABLED: toySyncFixture ? "true" : "false",
  DAILY_BRIEF_TOY_INGESTION_MODE: toySyncFixture ? "recovery" : "off"
};
...
progress(
  2,
  5,
  `isolated environment ready data_dir=${relativeDataDir} fixture=${fixturePath} ` +
  `fixture_audio=${requiresFixtureAudio ? "5525s_synthetic_silence" : "not_created"} spec=${requestedSpec}`
);
console.log(
  `[date-companion-e2e] providers transcription=fixture extraction=rule audio_insight=rule ` +
  `relationship=none emotion=none proactive=none memory_relevance=none hybrid=off`
);
...
try {
  ...
  const pipelineCompletionMarker = toySyncFixture
    ? "[pipeline] ready upload_id="
    : "[pipeline] background completed";
  if (requiresFixtureAudio && !serverOutput.includes(pipelineCompletionMarker)) {
